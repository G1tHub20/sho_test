...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  echo 'submit';
  for ($i = 0; $i < 5; $i++) {
    // $form[$i] = filter_input(INPUT_POST, 'no' . $i, FILTER_SANITIZE_STRING);
    $form[0] = filter_input(INPUT_POST, 'no0', FILTER_SANITIZE_STRING);
    $form[1] = filter_input(INPUT_POST, 'no1', FILTER_SANITIZE_STRING);
    $form[2] = filter_input(INPUT_POST, 'no2', FILTER_SANITIZE_STRING);
    $form[3] = filter_input(INPUT_POST, 'no3', FILTER_SANITIZE_STRING);
    $form[4] = filter_input(INPUT_POST, 'no4', FILTER_SANITIZE_STRING);
  }
  d($form);

  $subject = filter_input(INPUT_POST, 'subject', FILTER_SANITIZE_STRING); //←取得できていない
  // $subject = $_POST['subject']; //←取得できていない
  d($subject);
  $type = '穴埋め';
...
<?php include 'inc/header.php'; ?>
  ...
  <datalist id="example">
    ...
    <?php for ($i = 0; $i < 5; $i++): ?>
      ...
      <textarea name="no<?php echo $i; ?>" rows="5" cols="44" placeholder="<?php if ($i == 0) echo('1889年の[2月11日]に、大日本帝国憲法が発布された。'); ?>" required></textarea><br>
    <?php endfor; ?>
